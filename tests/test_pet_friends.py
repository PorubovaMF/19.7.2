...
def test_post_api_create_pet_simple(name='Кузя', animal_type='кот', age='2'):
...
def test_post_api_pets_set_photo(pet_photo = 'images/Cat2.jpeg'):
    """Проверяем, что можно добавить фото питомца по его ID"""
...
def test_post_api_create_pet_no_name(name='', animal_type='собака', age='1'):
    """Проверяем, что можно добавить питомца без имени"""

    # Запрашиваем ключ api и сохраняем в переменую auth_key
    _, auth_key = pf.get_api_key(valid_email, valid_password)

    # Если имя животного не указано, выкидываем исключение с текстом о невозможноти добавления питомца
    if name == '':
        ...
        status, result = pf.post_api_create_pet_simple(auth_key, name, animal_type, age, pet_photo)
        assert status == 200
        assert result['name'] == name

def test_get_api_key_incorrect_password(email=valid_email, password='valid_password'):
    """Проверяем, что запрос api ключа возвращает статус, отличный от 200 и в результате не содержится слово key"""
...
def test_get_all_pets_with_valid_key(filter='my_pets'):
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "ПИП", "ХОМЯК", "1")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
